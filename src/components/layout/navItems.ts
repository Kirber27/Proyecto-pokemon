...
export interface NavItem {
  label: string
  // Regiones y Perfil siguen fuera del MVP (CA-08.3), pero ahora navegan a un
  // placeholder de construcción en vez de quedar deshabilitados.
  routeName?: 'pokedex' | 'favorites' | 'regions' | 'profile'
  icon: Component
}
...
export const navItems: NavItem[] = [
  ...
  { label: 'Regiones', routeName: 'regions', icon: IconRegions },
  { label: 'Favoritos', routeName: 'favorites', icon: IconFavorites },
  { label: 'Perfil', routeName: 'profile', icon: IconProfile },
]
